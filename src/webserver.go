...
func (w *WebServer) handleAPs(resp http.ResponseWriter, req *http.Request) {
	...
	tmpl, err := template.New("aps.html").Funcs(funcMap).ParseFiles("./src/webserver/aps.html")
	if err != nil {
		http.Error(resp, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (w *WebServer) handleHomepage(resp http.ResponseWriter, req *http.Request) {
	tmpl, err := template.ParseFiles("./src/webserver/homepage.html")
	if err != nil {
		http.Error(resp, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (w *WebServer) handleProbes(resp http.ResponseWriter, req *http.Request) {
	...
	tmpl, err := template.New("probes.html").Funcs(funcMap).ParseFiles("./src/webserver/probes.html")
	if err != nil {
		http.Error(resp, err.Error(), http.StatusInternalServerError)
		return
	}
